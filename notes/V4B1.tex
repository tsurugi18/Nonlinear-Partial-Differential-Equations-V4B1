...
\item For any open subset $V\subset U$ which is compactly contained in $U$ (ie. $\overline{V}\subset U$), $\Vert u\Vert_{H^2(V)}\leq C(\Vert f\Vert_{L^2(U)}+\Vert u\Vert_{L^2(U)})$ for some $C>0$.
